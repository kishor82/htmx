...
// Handle Post request for user serach from jsonplaceholder
...
const getEmailTemplate = (value, info) => {
  return `<div class="mb-4" hx-target="this" hx-swap="outerHTML">
        ...
        <input
          ...
          required
        />
        <div class="${info.class}">${info.message}</div>
      </div>
  `;
};
...
app.post("/contact/email", (req, res) => {
  ...
  if (!emailRegex.test(submittedEmail)) {
    return res.send(getEmailTemplate(submittedEmail, inValid));
  } else {
    return res.send(getEmailTemplate(submittedEmail, isValid));
  }
});
